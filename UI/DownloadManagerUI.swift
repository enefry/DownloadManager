...
private struct GlobalActionsView: View {
    ...
    var body: some View {
        VStack {
            HStack {
                Text("任务: \(viewModel.counter.actives)/\(viewModel.counter.all)")
                    .font(.headline)
                    .foregroundColor(.secondary)
                    .frame(maxWidth: .infinity, alignment: .leading)
                Text("速度: \(viewModel.counter.actives)/\(viewModel.counter.all)")
                    .font(.headline)
                    .foregroundColor(.secondary)
                    .frame(maxWidth: .infinity, alignment: .leading)
            }
            HStack {
                Text("时间: \(viewModel.counter.actives)/\(viewModel.counter.all)")
                    .font(.headline)
                    .foregroundColor(.secondary)
                    .frame(maxWidth: .infinity, alignment: .leading)
                Text("进度: \(viewModel.counter.actives)/\(viewModel.counter.all)")
                    .font(.headline)
                    .foregroundColor(.secondary)
                    .frame(maxWidth: .infinity, alignment: .leading)
            }
...
//                        {action = ActionBlock(show: true, type: .resume, title: "恢复所有", message: "恢复所有下载？", action: )}
                ...
                Button(action: viewModel.manager.removeAllFinished
